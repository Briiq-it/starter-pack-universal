  ...
  class DeckStage extends HTMLElement {
    ...
    _render() {
      ...
      overlay.innerHTML = `
        ...
        <span class="divider"></span>
        <button class="btn present" type="button" aria-label="Modo apresentação" title="Apresentar (F) · S oculta sidebar">Apresentar<span class="kbd">F</span></button>
      `;
    ...
    _restoreIndex() {
      ...
      // state across loads. Also accepts ?_slide=<int> in the query string for
      // contexts where hashes don't round-trip reliably (e.g. overview thumbs
      // that share path-and-query and only differ by hash — some browsers
      // dedupe load events in that case).
      const params = new URLSearchParams(location.search);
      const qRaw = params.get('_slide');
      if (qRaw != null) {
        const n = parseInt(qRaw, 10) - 1;
    ...
    _setRailVisible(on, persist = true) {
      if (!this._railEnabled || this.hasAttribute('no-rail')) return;
    ...
    _togglePresent() {
      ...
      this._presentPrevRail = this._railVisible;
      this._setRailVisible(false, false);
      const root = document.documentElement;
      if (root && root.requestFullscreen) {
        root.requestFullscreen().catch(() => {
          // Browser refused — restore sidebar so user isn't stuck.
          if (this._presentPrevRail) this._setRailVisible(true, false);
          this._presentPrevRail = null;
        });
      }
    }
